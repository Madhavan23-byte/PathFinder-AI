...
export const SignupPage: React.FC = () => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      } else if (msg.includes('Failed to fetch') || msg.includes('NetworkError') || msg.includes('Server error')) {
        setApiError('Unable to connect to PathFinder server. Please make sure backend is running.');
      } else {
        setApiError(msg || 'Account registration failed. Please try again.');
      }
    } finally {
  ...
  return (
    <div className="min-h-screen bg-slate-900 text-white flex items-center justify-center p-4 sm:p-6 relative overflow-hidden font-sans selection:bg-indigo-500 selection:text-white">
      {/* Background Glow */}
      <div className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-[30rem] h-[30rem] bg-indigo-600/10 rounded-full blur-3xl pointer-events-none" />
